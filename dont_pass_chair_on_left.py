...
from pass_chair_on_right import all_chair_properties
from geometry_msgs.msg import PoseArray
...
class AsyncTrackingNode(Node):
    def __init__(self):
        ...
        self.chair_last_global_x = None
        self.chair_last_global_y = None
        ...
        self.active_properties = all_chair_properties
        ...
        # NEW: Debug Publisher to broadcast where the robot thinks the chair is
        self.debug_pub = self.create_publisher(Marker, '/debug/chair_tracked_position', 10)
    ...
    def set_ground_truth_from_camera(self, camera_angle):
        with self.lock:
            ...
            # NEW: Pull the current chair memory so the camera can check its math
            hg_x = self.chair_last_global_x
            hg_y = self.chair_last_global_y
        ...
        if best_match is not None:
            ...
            if hg_x is not None and hg_y is not None:
                ...
                # If the camera tries to teleport the chair further than your limit, block it!
            ...
            with self.lock:
                self.chair_last_global_x, self.chair_last_global_y = best_match
    ...
    def lidar_callback(self, msg):
        with self.lock:
            ...
            hg_x, hg_y = self.chair_last_global_x, self.chair_last_global_y
        ...
        # NEW: Predict where the chair is based on their current walking speed
        if dt > 0 and dt < 1.0:
            pred_x = hg_x + (self.chair_vx * dt)
            pred_y = hg_y + (self.chair_vy * dt)
        else:
        ...
        if current_time - last_time > self.memory_timeout:
            self.get_logger().warn("Track lost! chair out of range. Resetting graph.")
            self.frame_counter = 0
            with self.lock:
                self.chair_last_global_x = None
                self.chair_last_global_y = None
        ...
        if closest_distance <= self.max_jump_distance and best_global_x is not None:
            with self.lock:
                # NEW: Calculate how fast the chair is moving to update their momentum
                if dt > 0:
                    inst_vx = (best_global_x - hg_x) / dt
                    inst_vy = (best_global_y - hg_y) / dt
                    
                    # Smooth the velocity (50% old, 50% new) so it doesn't get jerky
                    self.chair_vx = (0.5 * inst_vx) + (0.5 * self.chair_vx)
                    self.chair_vy = (0.5 * inst_vy) + (0.5 * self.chair_vy)

                # Update memory with the new Global position
                self.chair_last_global_x = best_global_x
                self.chair_last_global_y = best_global_y
            ...
            # 4. Size (chair sized: 0.4m wide, 1.0m tall)
            ...
            if self.active_trackers is not None:
                for tracker in self.active_trackers:
                    ...
                    if tracker.is_trap() and not tracker.is_accepting():
                        ...
                        self.get_logger().info("Robot Successfully Passed on right!  !]")
                    else:
                        self.get_logger().info("Accepting State: chair is at ("+ str(round(self.chair_last_global_x*100)/100)+", "+str(round(self.chair_last_global_y*100)/100)+")")
    
    def build_networkx_graph(self, chair_x, chair_y):
        sg = nx.DiGraph()
        ego_node = SceneNode(node_id=0, name="ego", base_class="vehicle")
        chair_node = SceneNode(node_id=1, name="person_1", base_class="person")
        sg.add_node(ego_node)
        sg.add_node(chair_node)
        
        angle_rad = math.atan2(chair_y, chair_x)
        ...
        sg.add_edge(ego_node, chair_node, label=direction, type=direction, relation=direction)
